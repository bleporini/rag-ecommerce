...
resource "confluent_flink_statement" "product_keyed" {
  compute_pool {
    id = confluent_flink_compute_pool.main.id
  }
  principal {
  ...
  statement  = "insert into product_keyed select id_product, price from `fa560f9da14.prestashop.ps_product`;"
  properties = {
    "sql.current-catalog"  = confluent_environment.ecommerce.display_name
    "sql.current-database" = confluent_kafka_cluster.basic.display_name
  }
  ...
  depends_on = [
    confluent_flink_compute_pool.main,
    confluent_flink_statement.product_keyed_ddl,
    confluent_role_binding.app-manager-assigner,
    confluent_role_binding.app-manager-flink-developer
  ]
}

resource "confluent_flink_statement" "cart_product_priced_ddl" {
  compute_pool {
    id = confluent_flink_compute_pool.main.id
  }
  principal {
  ...
  statement  = <<-EOT
create table cart_product_priced (
  id_cart__id_product map<string, bigint> primary key not enforced,
price DECIMAL(20, 6),
quantity bigint
...
resource "confluent_flink_statement" "cart_product_priced" {
  compute_pool {
    id = confluent_flink_compute_pool.main.id
  }
  principal {
  ...
  statement  = <<-EOT
insert into cart_product_priced select
    map['id_cart', cp.id_cart,'id_product',cp.id_product] as id_cart__id_product,
    p.price,
    case when cp.__deleted = 'true' then 0 else cp.quantity end as quantity
from `fa560f9da14.prestashop.ps_cart_product` cp join product_keyed p on cp.id_product = p.id_product;
EOT
  ...
  depends_on = [
    confluent_flink_compute_pool.main,
    confluent_flink_statement.cart_product_priced_ddl,
    confluent_connector.source,
    confluent_role_binding.app-manager-assigner,
    confluent_role_binding.app-manager-flink-developer
  ]
...
resource "confluent_flink_statement" "cart_priced_ddl" {
  compute_pool {
    id = confluent_flink_compute_pool.main.id
  }
  principal {
...
resource "confluent_flink_statement" "cart_priced" {
  compute_pool {
    id = confluent_flink_compute_pool.main.id
  }
  principal {
  ...
  statement  = <<-EOT
insert into cart_priced select
    id_cart__id_product['id_cart'] as `_id`,
    sum(price * quantity)
from cart_product_priced
group by id_cart__id_product['id_cart'];
EOT
  ...
  depends_on = [
    confluent_flink_compute_pool.main,
    confluent_flink_statement.cart_priced_ddl,
    confluent_role_binding.app-manager-assigner,
    confluent_role_binding.app-manager-flink-developer
  ]
}
...
resource "confluent_connector" "sink" {
  ...
  "connection.user"           ="connect${random_id.id.id}"
  ...
  depends_on = [
    confluent_flink_statement.cart_priced,
